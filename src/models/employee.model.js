...
Employee.getEmployeeByID = (id, result)=>{
    dbConn.query('SELECT * FROM employee_table WHERE eid=?', id, (err, res)=>{
        if(err){
...
Employee.updateEmployee = (id, employeeReqData, result)=>{
    dbConn.query("UPDATE employee_table SET employee_name=?,employee_email=?,employee_phone_number=? WHERE eid = ?", [employeeReqData.employee_name,employeeReqData.employee_email,employeeReqData.employee_phone_number, id], (err, res)=>{
        if(err){
...
Employee.deleteEmployee = (id, result)=>{
    dbConn.query('DELETE FROM employee_table WHERE eid=?', [id], (err, res)=>{
         if(err){
